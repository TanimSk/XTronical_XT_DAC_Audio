...
void XT_DAC_Audio_Class::FillBuffer()
{
	...
	while (PlayItem != nullptr)
	{
		...
		while ((PlayItem->Playing) && (avail > 0)) // while space in buffer and item still active.  TEB, Sep-29-2019.
		{
			...
			if (AudioVolume > 100)
				AudioVolume = 100;									   // Constrain volume to 0-100%
...
XT_DAC_Audio_Class::XT_DAC_Audio_Class(uint8_t TheDacPin, uint8_t TimerNo, uint16_t PassedBufferSize)
{
	...
	timer = timerBegin(1000000);		   // Set timer frequency to 1 MHz (1 µs per tick)
	timerAttachInterrupt(timer, &onTimer); // Attach interrupt handler
	timerAlarm(timer, 20, true, 0);		   // Set up the alarm, enable it automatically, no reload count
